...
import com.cavox.models.ContactsModel;
...
public class ShareConatctsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public class ContactViewHolder extends RecyclerView.ViewHolder {
        CircularImageView conatctImage;
        ...
        protected Bitmap doInBackground(String... params) {
            ...
            try {
                if (params[0].equals("app")) {
                    ...
                    if (photo == null) {
                        photo = utils.loadContactPhoto(Long.parseLong(params[2]), mContext);
                    }
                } else if (params[0].equals("native")) {
                    photo = utils.loadContactPhoto(Long.parseLong(params[1]), mContext);
                } else if (params[0].equals("group") && photo == null) {
                    photo = CSDataProvider.getImageBitmap(params[1]);
                }

                if (photo == null) {
                    photo = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.defaultcontact);
                }
            } catch (Exception e) {
                try {
                    if (photo == null) {
