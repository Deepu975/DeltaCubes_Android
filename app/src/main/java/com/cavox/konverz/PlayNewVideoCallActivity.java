...
//GlobalVariables.lastcalltype = "video";
			...
			*/
            ...
            try {
                ...
                if (getIntent().getBooleanExtra("isinitiatior", false)) {
                    playConnectingTone();
                    destinationnumbettocall = getIntent().getStringExtra("dstnumber");
                    mycallid = CSCallsObj.startVideoCall(destinationnumbettocall, localRender, remoteRender,GlobalVariables.callrecord);
                    manageAudioFocus.requestAudioFocus(mycallid, destinationnumbettocall,"video");
                } else {
        ...
        public void onReceive(Context context, Intent intent) {
            try {
                ...
                if (mycallid.equals(intent.getStringExtra("callid"))) {
                    if (intent.getAction().equals(CSEvents.CSCALL_CALLENDED)) {
                        ...
                        Toast.makeText(getApplicationContext(), toasttext, Toast.LENGTH_LONG).show();
                            ...
                            else if (intent.getAction().equals(CSEvents.CSCALL_QUALITY)) {
                         if (mycallid.equals(intent.getStringExtra("callid"))) {
                            LOG.info("bweforvideo: call quality:"+intent.getIntExtra("callquality",6));
                        }
                    }
                ...
                if (intent.getAction().equals(CSEvents.CSCLIENT_NETWORKERROR)) {
                    stopringbacktone();
                    stopConnectingTone();
                    if(mediaconnectedatleastonce&&!utils.isinternetavailable(PlayNewVideoCallActivity.this)) {
                        reconnecting.setVisibility(View.VISIBLE);
...
*/
    ...
    public void onResume() {
        ...
        try {
            isonpause = false;
            LOG.info("RenderLayout: onResume");
            ...
            IntentFilter filter18 = new IntentFilter(CSEvents.CSCALL_QUALITY);
          ...
          /*  NotificationManager notificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
            ...
            //notificationid = utils.notifycallinprogress(getApplicationContext(), notoficationName, "Video call is in progress", "", "", 0);
            new ForeGroundServiceApis().startCallService(getApplicationContext(),notoficationName,"Video call is in progress");
