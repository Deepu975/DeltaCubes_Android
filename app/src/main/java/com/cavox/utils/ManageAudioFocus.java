...
//hold only if callid available and call is active-->sdk will take care.
    ...
    public boolean requestAudioFocus(String acallid, String aremoteid,String calltype) {
        try {

            LOG.info("GlobalVariables.incallcount:" + GlobalVariables.incallcount);

            if (GlobalVariables.incallcount <= 1) {
                am = (AudioManager) MainActivity.context.getSystemService(Context.AUDIO_SERVICE);
                //abandonAudioFocus();
                        ...
                        if (!callid.equals("") && !remoteid.equals("")) {
                            ...
                            switch (focusChange) {
                                case AudioManager.AUDIOFOCUS_GAIN:
                                    ...
                                    if (AppConstants.iSHoldCalled&&!AppConstants.isintentionalhold) {
